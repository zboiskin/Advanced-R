...
###BREAKOUT ROOM####
musk_tweets <- get_timeline('elonmusk', n=3200)

musk_tweets$created_at

musk_tweets <- musk_tweets %>%
  mutate(hour=hour(with_tz(created_at, tz="America/New_York")))

musk_hours <- musk_tweets %>%
  group_by(hour) %>%
  summarize(n=n()) %>% 
  mutate(freq=n/sum(n))
...
ggplot() +
  geom_col(data=obama_hours, mapping=aes(x=hour, y=freq), fill='blue', alpha=0.6) +
  geom_col(data=musk_hours, mapping=aes(x=hour, y=freq), fill='red', alpha=0.6)
